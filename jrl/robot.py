...
class Robot:
    ...
    def robot_sphere_collision_distance(self, x: torch.Tensor, spheres: torch.Tensor) -> torch.Tensor:
        """Returns the distance between the robot collision capsules and the provided spheres. Sphere should be in the
        format [n_spheres x 4] (center x, y, z, radius)

        Args:
            x (torch.Tensor): [n x ndofs] joint angle vectors
            spheres (torch.Tensor): [n_spheres x 4] spheres center and radius

        Returns:
            torch.Tensor: [n x n_spheres] distances
        """
